...
#       @element_attachment_prefs = attachment_prefs(@logical_parent.model, @local_model, 'element')
#       
...
# Temp disabled to test cursor & stack.
#
...
#       create_accessors = accessor?(@local_model) || create_accessors?(@model)
...
#       @logical_parent.object._attach_object!(new_object, label, delimiter?(@local_model), prefs, 'element', :default_class=>config[:default_class], :shared_variable_name=>shared_var_name, :create_accessors=>create_accessors)
#       
#       # if type == 'attribute'
...
#     def model_elements?(which=nil, _model=@model)
#       _model && _model.has_key?('elements') && ((_model['elements'] && which) ? _model['elements'].find{|e| e['name']==which} : _model['elements'])
#     end
