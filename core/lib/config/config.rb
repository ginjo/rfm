...
module Config
  ...
  def initialize(*args, **opts) #(opts={caller:self})
    ...
    # This works great but requires dependency on Refinements.
    #opts = opts.filter(AllowableOptions)
    # So here's the raw 'filter' function instead.
    opts = opts.select(){|k,v| AllowableOptions.flatten.include?(k.to_s)}
    @config ||= {}
    config(**opts)
